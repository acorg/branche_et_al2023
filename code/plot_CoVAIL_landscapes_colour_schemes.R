...
figure_dir <- file.path("figures",date,inf_stat, "col_scheme_landscapes", paste0(d_adj,"_b+o_adj"), paste0("optimization_", opti_nr))
...
# new angle for html page
angle <- list(
  rotation = c(-1.51, -0.0093, -0.171),
  translation = c(0, 0,0), #translation = c(0.0344, 0.0459, 0.1175),
  zoom = 1.45
)
  ...
  for(v1 in day_visno){
    ...
    if(v1 != last(day_visno)){
      ...
      for(v2 in day_visno[c((grep("TRUE", v1 == day_visno)+1):length(day_visno))]) {
        ...
        # make landscapes in different colours
        for(col_scheme in color_schemes){
         
          sr_group_colors <- read.csv(file.path(path_to_data, col_scheme), sep = ";", row.names = "Serum.group")
          arm_colors <- sr_group_colors[unique(sr_group_data$arm_code), "Color"]
          names(arm_colors) <- unique(sr_group_data$arm_code)
          ...
          lndscp_list[[paste0(inf_stat, "_", save_name)]] <- lndscp
