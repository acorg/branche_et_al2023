      ...
      if(v1 != last(day_visno)){
        for(v2 in c("D29", "D91")) {
          ...
          plots <- titerlineplot_dodge(sr_group_data_temp, sr_group_colors, titer_thresh = 40, antigens = plot_antigens,
                                 ...
                                 show_mean_line = T, mean_line_color = sr_group_colors["P", "Color"], 
                                 to_long = F,
                                 nrow_gmt = nrow_facet,
          ...
          ggsave(file.path(figure_dir, paste0(inf_stat, "_", v1, "_", v2, "_gmts_age_all_by_arm_show_p_arm.pdf")), plot = plots, dpi = 300, width =plot_width, height = 2+2*nrow_facet)
          
          plot_list[[v2]] <- plots
          
        }

        patchwork <- plot_list[[1]] / plot_list[[2]] + plot_annotation(tag_levels = 'A')  & 
          theme(plot.tag = element_text(size = 20))
        ggsave(file.path(figure_dir, paste0(inf_stat,  "_gmts_age_all_by_arm_show_p_arm.pdf")), plot = patchwork, dpi = 300, width =plot_width, height = 2*(2+2*nrow_facet))
        ...
        sr_group_data_temp <- sr_group_data_long %>%
          ...
          ggsave(file.path(figure_dir, paste0(inf_stat, "_D1_D29_D91_gmts_age_all_by_arm_colour_visit.pdf")), plot = plots, dpi = 300, width =plot_width, height = 2+2*nrow_facet)
